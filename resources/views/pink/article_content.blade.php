    ...
    <div class="hentry hentry-post blog-big group ">
        ...
        <div class="thumbnail">
            ...
            <div class="image-wrap">
                <img src="{{asset(env('THEME'))}}/images/articles/{{ $article->img->max }}" alt="00212" title="{{ $article->title }}" />
            </div>
            <p class="date">
        ...
        <div class="meta group">
            {{--ссылка должна перенаправить на страницу профиля пользователя, мы этого не делаем--}}
            <p class="author"><span>by <a href="#" title="{{ $article->title}}">{{ $article->user->name }}</a></span></p>
            <p class="categories"><span>In: <a href="{{ route('articlesCar',['cat_alias' => $article->category->alias]) }}" title="View all posts in {{$article->category->title}}" rel="category tag">{{$article->category->title}}</a></span></p>
